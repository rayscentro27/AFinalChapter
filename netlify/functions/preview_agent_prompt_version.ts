...
export const handler: Handler = async (event) => {
  try {
    ...
    const { agent_name, prompt_version, full } = QuerySchema.parse(qs);
    ...
    if (histErr || !hist) {
      // If the history table doesn't exist yet, PostgREST typically returns a 404-ish error.
    ...
    const systemPrompt = (hist.system_prompt || '').toString();
