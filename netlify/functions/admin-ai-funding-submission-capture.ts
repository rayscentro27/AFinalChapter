...
const BodySchema = z.object({
  ...
  client_device_confirmed: z.boolean(),
  confirmation_method: z.string().min(1).max(80).optional(),
  confirmation_metadata: z.record(z.any()).optional(),
...
export const handler: Handler = async (event) => {
  try {
    ...
    const proxied = await proxyToOracle({
      path: '/admin/ai/funding/submission-capture',
      method: 'POST',
      body,
      forwardAuth: true,
      event,
