...
export const handler: Handler = async (event) => {
  try {
    ...
    const model = routeModel({
      taskType: 'support_reply_suggestion',
      riskClass: 'medium',
    });
          ...
          parts: [
            {
              text: buildPrompt({
                tone: body.tone,
                count: body.count,
                subject: String((convo as any)?.subject || ''),
                status: String((convo as any)?.status || ''),
                summary: String((convo as any)?.summary_text || ''),
                transcript,
              }),
            },
          ],
    ...
    const parsed = parseSuggestions(rawText, body.count);
...
function buildPrompt(input: {
  ...
  return [
    'You are drafting staff reply suggestions for a secure client messaging inbox.',
    ...
    '- no mention that AI wrote the response',
    '- each suggestion 1-3 sentences',
    '',
    `Tone: ${input.tone}`,
    `Thread subject: ${input.subject || '(none)'}`,
    `Thread status: ${input.status || '(unknown)'}`,
    `Thread summary: ${input.summary || '(none)'}`,
    '',
...
function parseSuggestions(raw: string, desiredCount: number): string[] {
  const fallback = [
    'Thanks for the update. I reviewed your message and I am confirming the next steps on our side now.',
    'I can help with this today. Please confirm the best callback window and I will prioritize your request.',
    'Understood. I will post a status update in this thread shortly and outline any documents we still need.',
...
async function resolveStaffTenant(admin: any, userId: string, requestedTenantId: string | null): Promise<string> {
  ...
  if (requestedTenantId) {
    if (!allowedTenantIds.includes(requestedTenantId)) {
      throw statusError(403, 'requested_tenant_not_accessible');
    }
