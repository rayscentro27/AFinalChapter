...
begin
  if not exists (
    select 1 from pg_trigger where tgname = 'trg_strategy_library_updated_at'
  ) then
    create trigger trg_strategy_library_updated_at
    before update on public.strategy_library
