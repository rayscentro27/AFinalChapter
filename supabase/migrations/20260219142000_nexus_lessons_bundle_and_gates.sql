...
-- Additive migration; does not remove existing task pipeline.

create extension if not exists pgcrypto;
-- Enums requested by task board layer
DO $$ BEGIN
...
-- client_tasks compatibility columns used by bundle assignment + task board
...
-- Training data readable by authenticated users; mutable by master admin.
...
for all using (public.nexus_is_master_admin())
with check (public.nexus_is_master_admin());
-- Evidence should be tenant-scoped.
...
-- Ensure one default routing row exists.
