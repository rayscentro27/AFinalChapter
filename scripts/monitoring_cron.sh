...
MONITORING_WINDOW_MINUTES="${MONITORING_WINDOW_MINUTES:-15}"
...
log_json() {
  ...
  printf '{"ts":"%s","runner":"monitoring","level":"%s","tenant_id":"%s","status_code":"%s","message":"%s","response":%s}\n' \
...
for raw_tenant in "${TENANTS[@]}"; do
  ...
  payload=$(printf '{"tenant_id":"%s","window_minutes":%s}' "$tenant_id" "$MONITORING_WINDOW_MINUTES")
  tmp_file=$(mktemp)
  ...
  http_code=$(curl -sS \
    ...
    -o "$tmp_file" \
    -w "%{http_code}" \
    -X POST "${BASE_URL}/admin/monitoring/run" \
  ...
  if [[ "$http_code" =~ ^2 ]]; then
    log_json "info" "$tenant_id" "$http_code" "monitoring runner call succeeded" "${response_body:-{}}"
  else
    log_json "error" "$tenant_id" "$http_code" "monitoring runner call failed" "${response_body:-{}}"
  fi
done
