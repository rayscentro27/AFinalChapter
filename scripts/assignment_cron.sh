...
RUN_LIMIT="${ASSIGNMENT_RUN_LIMIT:-50}"
...
log_json() {
  ...
  printf '{"ts":"%s","runner":"assignment","level":"%s","tenant_id":"%s","status_code":"%s","message":"%s","response":%s}\n' \
...
for raw_tenant in "${TENANTS[@]}"; do
  ...
  http_code=$(curl -sS \
    ...
    -o "$tmp_file" \
    -w "%{http_code}" \
    -X POST "${BASE_URL}/admin/assignment/run" \
  ...
  if [[ "$http_code" =~ ^2 ]]; then
    log_json "info" "$tenant_id" "$http_code" "assignment runner call succeeded" "${response_body:-{}}"
  else
    log_json "error" "$tenant_id" "$http_code" "assignment runner call failed" "${response_body:-{}}"
  fi
