...
log_json() {
  ...
  printf '{"ts":"%s","level":"%s","tenant_id":"%s","status_code":"%s","message":"%s","response":%s}\n' \
...
for raw_tenant in "${TENANTS[@]}"; do
  ...
  http_code=$(curl -sS \
    ...
    -o "$tmp_file" \
    -w "%{http_code}" \
    -X POST "${BASE_URL}/admin/outbox/run" \
  ...
  if [[ "$http_code" =~ ^2 ]]; then
    log_json "info" "$tenant_id" "$http_code" "outbox runner call succeeded" "${response_body:-{}}"
  else
    log_json "error" "$tenant_id" "$http_code" "outbox runner call failed" "${response_body:-{}}"
  fi
