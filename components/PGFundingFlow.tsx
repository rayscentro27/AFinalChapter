...
import { CheckCircle, Circle, ArrowRight, Video, DollarSign, FileText, ShieldCheck, Loader, CreditCard, Play, Plus, Trash2, CalendarCheck, RefreshCw } from 'lucide-react';
...
const PGFundingFlow: React.FC<PGFundingFlowProps> = ({ contact }) => {
  ...
  const handlePayViaStripe = () => {
    ...
    if (!stripePk || stripePk === 'YOUR_STRIPE_PUBLIC_KEY') {
        alert("Payment Infrastructure Missing. Ensure your Stripe Public Key is configured in Settings.");
        return;
    }
    ...
    setTimeout(() => {
        setIsPaying(false);
        alert(`Payment of $${totalAmount.toLocaleString()} processed via Stripe (PK: ${stripePk.substring(0, 8)}...)`);
                ...
                {!showInvoice ? (
                   ...
                   <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 text-left max-w-sm mx-auto animate-fade-in">
                      ...
                      <button 
                        ...
                        disabled={isPaying}
                        className="w-full bg-slate-900 text-white font-bold py-4 rounded-xl hover:bg-slate-800 flex items-center justify-center gap-2 transition-all shadow-xl"
                      >
                         {isPaying ? <RefreshCw className="animate-spin" size={18}/> : <CreditCard size={18} />}
                         {isPaying ? 'Contacting Gateway...' : 'Pay via Stripe'} 
                      </button>
                      <p className="text-[10px] text-center text-slate-400 mt-4 uppercase tracking-widest font-bold">Secure Bank-Grade Transaction</p>
                   </div>
                )}
             </div>
           )}
