...
import { FileText, Sparkles, Printer, RefreshCw, AlertTriangle, CheckCircle, ShieldCheck, Download, Users, Image as ImageIcon } from 'lucide-react';
...
const CreditMemoBuilder: React.FC<CreditMemoBuilderProps> = ({ contacts, onUpdateContact }) => {
  ...
  const handleGenerateVisual = async () => {
    if (!selectedContact || !memo) return;
    setIsVisualizing(true);
    const visualUrl = await geminiService.generateEntityVisual(`${selectedContact.company} in the ${selectedContact.businessProfile?.industry} industry`);
    if (visualUrl && onUpdateContact) {
        const updatedMemo = { ...memo, visualUrl };
        setMemo(updatedMemo);
        onUpdateContact({ ...selectedContact, creditMemo: updatedMemo });
    }
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-fade-in pb-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 no-print">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium">Generate formal underwriting packets for lenders.</p>
        </div>
           ...
           <div className="lg:col-span-1 space-y-4 no-print">
              <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                 ...
                 <button 
                   ...
                   className="w-full bg-blue-600 text-white px-6 py-3 rounded-lg font-bold hover:bg-blue-700 shadow-lg flex items-center justify-center gap-2 mb-3 disabled:opacity-70"
                 >
                 ...
                 {memo && (
                    <button 
                        ...
                        className="w-full bg-indigo-600 text-white px-6 py-3 rounded-lg font-bold hover:bg-indigo-700 shadow-lg flex items-center justify-center gap-2 mb-3 disabled:opacity-70"
                    >
                        {isVisualizing ? <RefreshCw className="animate-spin" size={18}/> : <ImageIcon size={18}/>}
                        {memo.visualUrl ? 'Regenerate Visual' : 'Synthesize Visual'}
                    </button>
                 )}
                 ...
                 <div className="bg-white shadow-xl border border-slate-200 p-12 min-h-[800px] print:shadow-none print:border-none print:w-full">
                    ...
                    {memo.visualUrl && (
                        <div className="mb-10 rounded-2xl overflow-hidden shadow-lg border border-slate-100">
                            <img src={memo.visualUrl} alt="Entity Visualization" className="w-full h-64 object-cover" />
                            <div className="p-3 bg-slate-50 text-[10px] text-slate-400 font-bold uppercase tracking-widest text-center">
                                Neural Synthesis: Verified Business Context Visualization
                            </div>
                        </div>
                    )}
